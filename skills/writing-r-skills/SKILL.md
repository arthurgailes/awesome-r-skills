...
> Adapted from [superpowers](https://github.com/obra/superpowers) by Jesse Vincent. MIT License.
...
Concrete method with steps to follow (debugging-pipes, profiling-with-profvis)

### Pattern

Way of thinking about problems (tidy-data, functional-programming)

### Reference

API docs, syntax guides, tool documentation (r-ggplot2, r-data-table)
...
- Describe the _problem_ (race conditions, inconsistent behavior) not _language-specific symptoms_ (Sys.sleep, delays)
...
description: Use when tests use Sys.sleep and are flaky

# ✅ GOOD: Starts with "Use when", describes problem, no workflow
description: Use when tests have race conditions, timing dependencies, or pass/fail inconsistently

# ✅ GOOD: Technology-specific skill with explicit trigger
description: Use when using Shiny and handling authentication redirects
```
...
- Error messages: "Error in", "could not find function", "subscript out of bounds"
- Symptoms: "flaky", "hanging", "memory", "slow"
- Synonyms: "timeout/hang/freeze", "cleanup/teardown/on.exit"
...
- ✅ `debugging-pipes` not `pipe-debug-helpers`
...
```r
# ❌ BAD: Document all arguments in SKILL.md
fread supports sep, header, na.strings, select, drop, colClasses...

# ✅ GOOD: Reference help
fread supports many options. Run ?fread for details.
```
...
your human partner: "How did we handle authentication errors in Shiny before?"
You: I'll search past conversations for Shiny authentication patterns.
[Dispatch subagent with search query: "Shiny authentication error handling 401"]

# ✅ GOOD: Minimal example (20 words)

Partner: "How did we handle auth errors in Shiny?"
You: Searching...
...
- ✅ `debugging-pipes` > `pipe-debug-helpers`
- ✅ `using-skills` not `skill-usage`
- ✅ `tidy-reshaping` > `data-structure-refactoring`
- ✅ `root-cause-tracing` > `debugging-techniques`

**Gerunds (-ing) work well for processes:**

- `creating-skills`, `testing-skills`, `debugging-with-browser`
...
Choose most relevant approach for R:

- Data manipulation → data.table or dplyr
- Visualization → ggplot2
- Debugging → browser(), trace(), profvis
- Testing → testthat
...
debugging-pipes/
  ...
  example.R   # Working helpers to adapt
```
...
r-ggplot2/
  ...
  geoms.md       # 600 lines geom reference
  scales.md      # 500 lines scale reference
...
**Examples:** debugging-pipes, root-cause-tracing, defensive-programming
...
**Examples:** tidy-data, functional-programming concepts
...
**Examples:** r-ggplot2, r-data-table, package guides
...
example-r.R, example-py.py, example-js.js
**Why bad:** Mediocre quality, maintenance burden

### ❌ Code in Flowcharts

```dot
step1 [label="library(dplyr)"];
